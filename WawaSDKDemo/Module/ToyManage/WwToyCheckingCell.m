...
static NSString *Bottom_UnSelectButtonString = @"toy_checking_unselect";
static NSString *Bottom_SelectButtonString = @"toy_checking_select";

@interface WwToyCheckingCell ()
{
...
@implementation WwToyCheckingCell
...
- (void)reloadDataWithModel:(WwWawaDepositModel*)model {
    self.model = model;
    [self.image sd_setImageWithURL:[NSURL URLWithString:realString(model.pic)]];
    
    self.name.text = model.name;
    self.priceHint.text = [NSString stringWithFormat:@"可兑金币:"];
    NSMutableAttributedString *imgStr = [UILabel labelAttributedString:[NSString stringWithFormat:@"%zi", model.coin] spec:@" " imageName:@"toy_exchange_coin" point:CGPointMake(2, -2) imgFrontString:NO];
...
- (void)updateConstraints {
    ...
    [self.name mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self).with.offset(16.f);
    ...
    [self.priceHint mas_makeConstraints:^(MASConstraintMaker *make) {
        @strongify(self);
        make.left.equalTo(self.bgView).with.offset(135.f);
        make.top.equalTo(self.name.mas_bottom).with.offset(10.f);
...
- (UIImageView *)dateImg {
    if (!_dateImg) {
        ...
        _dateImg.image = [UIImage imageNamed:@"toy_checking_countdown"];
...
- (UILabel *)date {
    if (!_date) {
        ...
        _date.textColor = RGBCOLOR(174, 174, 174);
    }
    return _date;
}
@end
