...
//全屏的
#define TopColorArr  @[(id)DVLColorGen(@"#ff9600").CGColor, (id)DVLColorGen(@"#fff660").CGColor]
...
@implementation ZYCountDownView
...
{
    self.status = ZYCountDownStatusCountDown;
...
- (void)setStatus:(ZYCountDownStatus)status
{
    safe_async_main(^{
        _status = status;
        if (status == ZYCountDownStatusCountDown) {
            self.hidden = NO;
            self.progressView.hidden = NO;
            self.countLabel.hidden = NO;
        }
        else if (status == ZYCountDownStatusCountFinish){
            [self stopTimer];
        ...
        else if (status == ZYCountDownStatusRequestResultIng) {
            [self stopTimer];
            self.countLabel.text = @"等待\n结果";
            self.hidden = NO;
            self.progressView.hidden = YES;
        ...
        else if (status == ZYCountDownStatusRequestComplete) {
            [self stopTimer];
            self.hidden = YES;
        }
...
- (void)startTimer
{
    ...
    self.countTimer = [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(countDown) userInfo:nil repeats:YES];
...
- (void)countDown
{
    ...
    if (_countNum == 0) {
        [self stopTimer];
        self.status = ZYCountDownStatusCountFinish;
...
{
    if (!_countLabel) {
        _countLabel = [[UILabel alloc] initWithFrame:(CGRect){0,0,CGRectGetWidth(self.frame),CGRectGetHeight(self.frame)}];
...
- (CALayer *)innerShader
{
    if (!_innerShader) {
        ...
        _innerShader.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.2].CGColor;
...
- (void)updateProgress:(CGFloat)progress
{
    ...
    if (progress == 1) {
        self.topLayer.colors = @[(id)DVLColorGen(@"#ff9600").CGColor, (id)DVLColorGen(@"#fff660").CGColor];
...
{
    if (!_colorLayer) {
        CGRect frame =  (CGRect){0,0,CGRectGetWidth(self.frame),CGRectGetHeight(self.frame)};
