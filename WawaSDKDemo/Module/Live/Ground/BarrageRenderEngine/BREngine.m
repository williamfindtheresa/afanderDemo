...
- (void)checkShowingElement //can overrite
{
    ...
    [_showingQueue enumerateObjectsUsingBlock:^(id<BRElementInterface>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (!cv) {
            ...
            if (frame.origin.x > CGRectGetWidth(canvas.frame)/2.0) {
                //认为此航道不可用
...
- (void)normalWalkAnimateLayout:(id<BRElementInterface>)element withElementView:(UIView *)elementView toContainerView:(BRElementContainerView *)containerView inLine:(NSInteger)line /**< 普通从右到左 过场动画*/
{
...
- (void)startCheckTimer
{
    ...
    _checkTimer = [NSTimer scheduledTimerWithTimeInterval:val
                                                   target:self
                                                 selector:@selector(checkShowingElement)
